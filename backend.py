...
class TinyYoloFeature(BaseFeatureExtractor):
    ...
    def __init__(self, input_size):
        ...
        # Layer 1
        x = Conv2D(
            16, (3, 3), strides=(1, 1), padding="same", name="conv_1", use_bias=False
        )(input_image)
        x = BatchNormalization(name="norm_1")(x)
        x = LeakyReLU(alpha=0.1)(x)
        x = MaxPooling2D(pool_size=(2, 2))(x)
        ...
        # Layer 6
        x = Conv2D(
            512, (3, 3), strides=(1, 1), padding="same", name="conv_6", use_bias=False
        )(x)
        x = BatchNormalization(name="norm_6")(x)
        x = LeakyReLU(alpha=0.1)(x)
...
class SqueezeNetFeature(BaseFeatureExtractor):
    ...
    def __init__(self, input_size):
        ...
        x = Conv2D(64, (3, 3), strides=(2, 2), padding="valid", name="conv1")(
            input_image
        )
