...
class YOLO(object):
            ...
            for col in range(grid_w):
                for b in range(nb_box):
                    ...
                    if np.sum(classes) > 0:
                        ...
                        w = (
                            self.anchors[2 * b + 0] * np.exp(w) / grid_w
                        )  # unit: image width
                        h = (
                            self.anchors[2 * b + 1] * np.exp(h) / grid_h
                        )  # unit: image height
            ...
            for i in range(len(sorted_indices)):
                ...
                if boxes[index_i].classes[c] == 0:
                    ...
                    for j in range(i + 1, len(sorted_indices)):
                        ...
                        if (
                            self.bbox_iou(boxes[index_i], boxes[index_j])
                            >= nms_threshold
                        ):
    ...
    def train(
        ...
        early_stop = EarlyStopping(
            monitor="val_loss", min_delta=0.001, patience=2, mode="min", verbose=1
        )
        # checkpoint
